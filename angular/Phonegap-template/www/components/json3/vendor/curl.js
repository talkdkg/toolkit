    ...
    createResourceDef: function (id, cfg, isPreload, optCtxId) {
      ...
      function localRequire(ids, callback) {
        ...
        cb = callback && function () {
          callback.apply(undef, arguments[0]);
        };
...
//					info.specificity = (id.match(findSlashRx) || []).length;
      ...
      function convertPathMatcher(cfg) {
        ...
        cfg.pathRx = new RegExp('^(' +
            cfg.pathList.sort(function (a, b) {
              return pathMap[a].specificity < pathMap[b].specificity;
            })
                .join('|')
                .replace(/\//g, '\\/') +
            ')(?=\\/|$)'
    ...
    fixArgs: function (args) {
      ...
      return {
        id: id,
        deps: deps || [],
        res: isDefFunc ? defFunc : function () {
          return defFunc;
        },
        cjs: cjs
      };
